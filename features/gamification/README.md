...
## Typography — 7 ขั้น ทุกค่ามาจาก token

ตรวจทั้งฟีเจอร์ 2026-08-24 · ไม่เหลือเลขดิบใน CSS แม้แต่ตัวเดียว

| ขั้น | role | ใช้กับ |
|---|---|---|
| 20/36 w600 | `display-xl-semibold` | ชื่อรางวัล · ตัวเลขความคืบหน้าตัวใหญ่ |
| 16/24 w600 | `title-lg-semibold` | หัวข้อทุกบล็อก · รางวัลบนการ์ดที่ปิดแล้ว |
| 14/22 w600 | `body-md-semibold` | ชื่อรางวัลในลิสต์หมุด |
| 14/22 w400 | `body-md-regular` | เงื่อนไข · ขั้นตอน · empty state |
| 12/18 w600 | `label-md-semibold` | **ค่า**ของ fact · ตัวนับใน track · สถานะหมุด |
| 12/18 w500 | `label-md-medium` | ชื่อภารกิจบนการ์ด |
| 12/18 w400 | `caption-lg-regular` | label · caption · note · เงื่อนไขท้าย · ตัวเลขบนราง |
| 10/16 w600 | `caption-md-regular` + weight | eyebrow ประเภทรางวัลเท่านั้น |

**เหตุผลที่ต้องอธิบาย**

- **20px ต้องใช้ leading 36 ไม่ใช่ 26/28 ที่เคยเขียนเอง** — วัดจริง Graphik TH 20px/600 ได้ fontBox 24.0px
  แต่คลัสเตอร์ไทยที่มีสระบน + วรรณยุกต์ซ้อน ink จริง **27.6px** → 24 หนีบ · 26 ก็ยังไม่พอ
  36 เป็นขั้นเดียวในสเกลที่พ้น และนี่คือเหตุผลที่ DS ให้ 20px ทุก role เป็น 36
- **เอา `letter-spacing: 0.08em` ออกจาก eyebrow ภาษาไทย** — ไทยไม่เว้นวรรคระหว่างคำ และสระ/วรรณยุกต์
  เป็นก้อนเดียวกับพยัญชนะ การถ่างตัวอักษรทำลายก้อนที่ตาไทยกำลังหา · DS เห็นด้วย — tracking token ทั้ง 35 ตัวเป็น 0
- **ค่าที่ยังไม่ยืนยันเลิกอยู่ที่ 10–11px** — "รอข้อมูล" คือ*คำตอบ*ของแถว ไม่ใช่ eyebrow → 12/18 เท่ากับค่าที่ยืนยันแล้ว
- **เลิกใช้ monospace มาร์คค่าที่ยังไม่ยืนยัน** `[user 2026-08-24]` — `ui-monospace`/`SFMono`/`Menlo` **ไม่มีตัวไทย**
  เบราว์เซอร์เลย substitute ฟอนต์ไทยอะไรก็ได้ของเครื่อง → ข้อความออกมาเป็น typeface ที่ไม่มีที่อื่นในแอป และเปลี่ยนตามเครื่อง
  (วัดได้ 42.79px เทียบ Graphik TH 44.41px สำหรับข้อความเดียวกัน) · เป็น convention ของ typography ฝรั่งที่ภาษาไทยรับไม่ได้
  **สัญญาณย้ายไปอยู่ที่ชิปสีเตือน** — ใช้แบบเดียวกันทั้งการ์ดและหน้า detail: หนึ่งความหมาย หนึ่งการแสดงผล
- **`bindUnits()`** — ผูกตัวเลขกับหน่วยด้วย `\u00A0` ไม่ให้ "รับได้หมุดละ 1" / "ครั้งต่อบัญชี" ขาดคนละบรรทัด
  เขียนเป็นกฎ ไม่ใช่พิมพ์ nbsp ลงข้อความ เพราะ nbsp มองไม่เห็นใน diff แล้วจะหายตอนใครแก้ประโยค

`tools/contrast-audit.js` — 0 failure ทั้ง list · detail · empty · prototype
