...
| | ↳ VOLUME ขั้นล่าง = **บันได 3 หมุด มีรางวัลทุกหมุด** (หมุดกว้างเท่ากัน ไม่ใช่ตามสเกล) · VOLUME ขั้นบน = `X/Y` เส้นเดียวไม่มีหมุด · **FREQUENCY = 2 เส้นแยกกัน** (งวด + Jidrid) | เงื่อนไขคนละชนิดรวมเป็นเส้นเดียวไม่ได้ — ผู้ใช้จะอ่านไม่ออกว่าค้างที่อันไหน (prd MT-02) |
| | ↳ บันไดวางหมุดกว้างเท่ากันเพราะ 50 · 200 · 500 ถ้าวาดตามสเกลจะกองอยู่ซ้ายสุด คนที่ได้ 38 ใบเห็นแถบว่างเปล่าทั้งที่เหลืออีก 12 ใบถึงรางวัลแรก — ตัวเลขกำกับทุกหมุดจึงเป็นตัวบอกระยะจริง | prd MT-03 |
| | ↳ task แบบ **ทำ/ไม่ทำ** (เช่น "ใช้จิ๊ดริดกล่องสุ่ม 3") วาดเป็น**เครื่องหมายถูก ไม่ใช่แถบ** | แถบบอกตำแหน่งระหว่างทาง แต่สิ่งนี้ไม่มีระหว่างทาง |
...
```
┌ Hero ──────────────┐  รางวัล + ช่วงเวลาแคมเปญ
├ Progress ──────────┤  ตามกลุ่มภารกิจ — บันได · เส้นเดียว · 2 เส้น (prd §6.0)
├ เงื่อนไข ───────────┤  ชื่อภารกิจ + สิ่งที่ต้องทำ — บันไดใช้ "รางวัลแต่ละหมุด" แทน
├ โควตา & เงื่อนไข ───┤  จำนวนคงเหลือ · อายุคูปอง · เงื่อนไขของรางวัลสินค้า
└ CTA ───────────────┘  ปุ่มเดียว เปลี่ยนตามสถานะ
```

**บล็อกเงื่อนไขมีได้แบบเดียวต่อหน้า — ไม่พูดซ้ำ**

| กลุ่ม | บล็อกที่ใช้ | ทำไม |
|---|---|---|
| VOLUME #1 (บันได) | **รางวัลแต่ละหมุด** — รูป + ถึงกี่ใบ + รับแล้ว/รอรับ/ยังไม่ถึง | หมุดคือเงื่อนไข และตอบ "ได้อะไรตรงไหน" ในบล็อกเดียว |
| VOLUME #2 (task เดียว) | **เงื่อนไข** 1 บรรทัด | มีอย่างเดียวให้ทำ |
| FREQUENCY (2 task) | **2 เส้นในการ์ด progress** — ไม่มีบล็อกเงื่อนไขซ้ำอีก | เส้นทั้งสองบอกครบแล้วว่าต้องทำอะไรและค้างตรงไหน |
