...
## 2026-08-23 · เจอ mock ไม่ตรงบรีฟแล้วสร้างต่อ แทนที่จะหยุดถาม
- **เกิดอะไร:** บรีฟบอกว่า Figma `7gVv3oV6G6xzldSjIxoSxb` มี frame ชื่อ `MSN-xxx` ตรงกับ ux-gamification.md · ค้นจริงได้ **0 ตัว** (ชื่อจริงคือ `Home` · `mission_detail` · `Mission_Main-empty`) และไฟล์เป็น **hi-fi** ทั้งที่ ticket T1 ระบุ mid-fi · ผมแมป frame → MSN เอง แล้วสร้างต่อจนได้ 1 หน้า + 3 component + export asset 8 ไฟล์ ค่อยรายงาน — user สั่งหยุดกลางทางแล้วเปลี่ยนเป็น "ไม่อ้างอิง Figma เลย" ของที่ทำทิ้งทั้งหมด
- **ทำไม:** เอา "รายงานตอนจบ" ไปแทน "หยุดตอนเจอ" — ตอนที่ค้น MSN ได้ 0 ตัวคือจุดที่ **สมมติฐานหลักของงานพัง** ไม่ใช่รายละเอียดที่เก็บไว้เล่าทีหลัง · การแมปเองเป็นการตัดสินใจแทน designer ว่า frame ไหนคือหน้าจอไหน ซึ่งเกินสิทธิ์ที่บรีฟให้มา
- **ครั้งหน้าทำยังไง:** ถ้า artifact ที่บรีฟอ้างถึง **หาไม่เจอ หรือชนิดไม่ตรงที่บอก** (ชื่อ frame · fidelity · เวอร์ชัน) ให้หยุดทันทีแล้วถามก่อนเขียนไฟล์แรก — ไม่ใช่แค่ verify แล้วเดินต่อ · เกณฑ์แยก: ค่าที่วัดผิดคาด = แก้เองได้ · **ตัวตนของ input ผิดคาด = ต้องให้คนตัดสิน**

## 2026-08-23 · typography ผูก viewport ไม่ใช่ container — รีวิวมือถือใน Storybook ได้ขนาดผิด
- **เกิดอะไร:** วัด subtitle ในการ์ดได้ `14px/22` ทั้งที่ role `sub-title-md-regular` = 12/18 → ข้อความ 1 บรรทัดกลายเป็น 2 บรรทัด เกือบสรุปว่าเขียน layout ผิด
- **ทำไม:** `ui/foundations/tokens.css` สลับ typography เป็นชุด desktop ที่ `@media (min-width: 768px)` บน `:root` — หน้าจอมือถือที่วางใน `DeviceFrame` กว้าง 393 ก็จริง แต่ **viewport ของ browser** กว้าง 1400 media query เลยติด ทั้งที่กำลังรีวิวหน้าโทรศัพท์
- **ครั้งหน้าทำยังไง:** ก่อนวัด/เทียบขนาดของหน้าจอมือถือใน Storybook ให้ย่อ browser ให้ **ต่ำกว่า 768px** ก่อนเสมอ แล้วค่อยอ่านค่า — และเวลาเห็น font-size ไม่ตรง role ให้เช็ค `innerWidth` ก่อนโทษโค้ด (ตระกูลเดียวกับบทเรียน `document.hidden` 2026-08-20)

## 2026-08-23 · `<button>` ที่เป็น flex container ไม่ stretch ลูกทุก engine
- **เกิดอะไร:** การ์ดภารกิจเป็น `<button>` ตั้ง `display:flex; flex-direction:column; width:100%` — บนเครื่องผม (Chromium ใน Browser pane) แถบรางวัลแดงเต็มความกว้าง 361px ทุกชั้น วัดแล้วตรงหมด · แต่บนเครื่อง user แถบแดงกว้างแค่ ~65% ของการ์ด เหลือขอบขาวด้านขวา — ลูกทุกตัวหดตามเนื้อหาแทนที่จะเต็ม
- **ทำไม:** UA stylesheet บาง engine ตั้ง `align-items: center` ให้ `button` — พอ button กลายเป็น flex container ค่านั้นยังอยู่ ลูกเลยได้ `align-self: center` = shrink-to-fit ไม่ใช่ stretch · `getComputedStyle` บน Chromium คืน `align-items: normal` (= stretch) จึงวัดยังไงก็ไม่เจอ **บั๊กนี้มองไม่เห็นเลยจากเครื่องที่ไม่มีปัญหา** (ตระกูลเดียวกับ Tailwind preflight 2026-08-20 — default ที่มองไม่เห็นในโค้ดที่ยกมา)
- **ครั้งหน้าทำยังไง:** element ที่ browser มี UA style ของตัวเอง (`button` `fieldset` `summary` `input`) แล้วเราตั้ง `display:flex/grid` ทับ — ต้องเขียน `align-items` กับ `justify-content` ให้ครบทุกครั้ง ห้ามพึ่ง initial value · และเวลา user บอกว่า layout เพี้ยนแต่เราวัดแล้วปกติ ให้สงสัย **UA stylesheet ต่าง engine** ก่อนสงสัยว่า user ดูผิดหน้า

## 2026-08-23 · ยกสีจากดีไซน์มาใช้โดยไม่วัด contrast
- **เกิดอะไร:** import ดีไซน์จาก Claude Design แล้วยกค่าสีมาตรงๆ ทุกตัว เพราะ "ทุกสี resolve เป็น `--sys-*` ที่มีอยู่แล้ว" — user ทักว่า contrast ไม่ผ่าน พอวัดจริงเจอ fail 6 จุด: ตัวอักษรชมพู `#FFE2E2` บนแดง `#E32321` = 3.80 (ต้อง 4.5) · การ์ดที่ปิด `#A3A3A3` บน `#F5F5F5` = 2.31 · ติ๊กขาวบน `#22C55E` = 2.28 (icon ต้อง 3.0) · หมุด ring `#D4D4D4` บนขาว = 1.48
- **ทำไม:** เอา "สีมาจาก token ที่ approve แล้ว" มาเป็นหลักประกันว่าใช้ได้ — แต่ **token บอกว่าสีนี้มีอยู่ในระบบ ไม่ได้บอกว่าคู่นี้อ่านออก** contrast เป็นคุณสมบัติของ *คู่สี* ไม่ใช่ของสีเดี่ยว · และดีไซน์ที่ import มาก็ไม่ได้ถูกตรวจ ไม่มีใครในเส้นทางนี้วัดเลยสักคน
- **ครั้งหน้าทำยังไง:** ทุกครั้งที่วางคู่ foreground/background ใหม่ (โดยเฉพาะตอน import ดีไซน์จากที่อื่น) ให้รันตารางวัด ratio ของ **ทุกคู่ที่ใช้จริง** ก่อน commit — เกณฑ์ 4.5 ตัวอักษรปกติ · 3.0 ตัวอักษร ≥24px หรือ ≥19px bold · 3.0 icon กับกราฟิกที่สื่อความหมาย · disabled ยกเว้นได้แต่ก็ควรอ่านออก · และเวลาสีอ่อนบนพื้นแบรนด์ไม่ผ่าน ให้ **เข้มพื้นลง** ก่อนคิดจะทิ้งลำดับชั้นของสี

## 2026-08-23 · จัดสีให้ DS component ด้วย className แล้วไม่ติด
- **เกิดอะไร:** ตั้ง `.ltp-mission-block__hero-meta { color: var(--sys-color-primary-light) }` แล้วส่งเข้า `<Text className="ltp-mission-block__hero-meta">` — สีไม่เปลี่ยน render ออกมาเป็น `#262626` บนแดงเข้ม `#991B1B` = **1.94:1** อ่านแทบไม่ออก และหลุดไป 1 commit เต็มๆ ทั้งที่เพิ่ง commit fix contrast ไปรอบก่อน
- **ทำไม:** `ui/components/Text` เขียน `color` เป็น **inline style** (`style={{ color: var(--sys-color-text-...) }}`) — inline ชนะ class เสมอไม่ว่าจะเขียน selector ยังไง · class เลยไม่เคยมีผล และไม่มีอะไร throw · ตอนตรวจผมดูว่า "ตั้ง class ไว้แล้ว" แทนที่จะดู computed color
- **ครั้งหน้าทำยังไง:** จะเปลี่ยนสี/ฟอนต์ของ DS component ที่เขียน style แบบ inline (`Text` `Badge` `Button`) ต้องส่งผ่าน **prop `style`** เท่านั้น (ของ `Text` spread `...style` ท้ายสุด จึงชนะ) หรือใช้ `tone` ที่มีอยู่ — ห้ามใช้ className · และเวลาเช็คงานสี ให้อ่าน `getComputedStyle(el).color` ทุกครั้ง ไม่ใช่ดูว่าเขียน class ไว้หรือยัง

## 2026-08-23 · เขียน audit เป็นสคริปต์ ดีกว่าวัดด้วยตารางที่พิมพ์เอง
- **เกิดอะไร:** รอบแรกผมวัด contrast ด้วยการพิมพ์คู่สีลงตาราง Python เอง — เจอ 6 จุด แก้แล้วบอกว่าครบ · user ทักอีกรอบ พอเขียนสคริปต์เดินทุก element ใน DOM จริงกลับเจออีก 5 จุดที่ตารางไม่มี (hero meta · ลิงก์ 3 ปุ่ม · แท็บ active) เพราะผมพิมพ์เฉพาะคู่ที่ *คิดว่า* มี
- **ทำไม:** ตารางที่พิมพ์มือครอบคลุมเท่าที่จำได้ ไม่ใช่เท่าที่ render จริง — และมันไม่รู้เรื่อง background ที่ inherit มาจาก ancestor (แท็บโปร่งใสทับ `#FAFAFA` ไม่ใช่ขาว)
- **ครั้งหน้าทำยังไง:** งานตรวจที่นับได้ (contrast · touch target · ขนาดฟอนต์ · alt text) ให้เขียนเป็นสคริปต์ที่เดิน DOM จริงตั้งแต่ครั้งแรก แล้ว commit ไว้ให้รันซ้ำได้ — อยู่ที่ `tools/contrast-audit.js` · ห้ามส่งงานโดยยืนบน checklist ที่พิมพ์เอง

## 2026-08-24 · หมุดที่วางเป็นคอลัมน์เท่ากัน = กราฟที่โกหกตำแหน่ง
- **เกิดอะไร:** `MissionProgress` วางหมุด `10 · 25 · 40 · 50` (เป้า 50) เป็น flex column กว้างเท่ากัน → หมุดไปอยู่ที่ 12.5% · 37.5% · 62.5% · 87.5% ทั้งที่ค่าจริงคือ 20% · 50% · 80% · 100% · ตัวเลขที่พิมพ์ใต้หมุดถูก แต่ตำแหน่งผิด อยู่รอด 3 commit เพราะ "ดูสวยดี" และไม่มีใครวัด
- **ทำไม:** ลอกโครงจากดีไซน์ที่วาดหมุดห่างเท่าๆ กัน แล้วแปลตรงๆ เป็น `flex: 1` — ดีไซน์วาดด้วยมือจึงเว้นเท่ากันโดยไม่ได้คิดเรื่องสเกล แต่พอเป็นโค้ดที่รับ `marks[]` เป็นข้อมูล มันกลายเป็นการ plot ค่าลงแกน ซึ่งต้องเคารพสเกล
- **ครั้งหน้าทำยังไง:** อะไรก็ตามที่ **วางของลงบนแกนที่มีหน่วย** (progress · timeline · แกนกราฟ · ruler) ตำแหน่งต้องคำนวณจาก `value / max` เสมอ ห้ามใช้ `flex: 1` หรือ `space-between` แจกช่องเท่ากัน — และเวลารีวิวให้วัดตำแหน่งจริงเทียบกับที่ควรเป็น (`(left + w/2 - railLeft) / railWidth`) ไม่ใช่ดูว่าเรียงสวยไหม

## 2026-08-24 · `position: absolute; inset: 0` ผูกกับ ancestor ที่ positioned ไม่ใช่พี่น้องที่อยู่ข้างๆ
- **เกิดอะไร:** วาง `.scrim` (เฟดรูป→แดง) เป็น **sibling** ของกล่องรูป โดยเข้าใจว่ามันจะกางเท่ากล่องรูป — จริงๆ `inset: 0` ไปเทียบกับ `.hero` ที่เป็น `position: relative` ตัวใกล้สุด scrim เลยสูง **295** แทนที่จะเป็น **181** เฟดจึงทึบสุดที่ก้นบล็อกข้อความ ส่วนตรงรอยต่อรูป-ข้อความเพิ่งไปถึง ~6% แล้วชนแดงทึบ = **เห็นเป็นเส้น** · จูน gradient ไป 3 รอบ (หนักไป → เบาไป → eased) ไม่มีทางแก้ได้เลยเพราะจูนเฟดที่จบผิดที่
- **ทำไม:** อ่านโค้ดแล้วเห็น `.art { position: relative }` กับ `.scrim { inset: 0 }` อยู่ใกล้กันใน CSS เลยเหมาเอาว่าผูกกัน — แต่ความสัมพันธ์ absolute อยู่ที่ **DOM tree** ไม่ใช่ที่ลำดับใน stylesheet · และผมเขียนคอมเมนต์ยืนยันความเข้าใจผิดนั้นลงไปด้วย ("the fade lives INSIDE the picture's box") ทำให้รอบต่อๆ มาไม่กลับมาสงสัยตรงนี้อีก
- **ครั้งหน้าทำยังไง:** เวลา overlay ไม่พอดีกับของที่ตั้งใจให้มันคลุม ให้วัด `getBoundingClientRect()` ของ **overlay เทียบกับเป้า** ก่อนแตะค่าสีหรือ gradient — ถ้าขนาดไม่เท่ากันคือปัญหาโครงสร้าง ไม่ใช่ปัญหาจูน · และ overlay ที่ต้องคลุม element ไหน **ต้องเป็นลูกของ element นั้น** ไม่ใช่พี่น้อง

## 2026-08-24 · ชนกับดัก inline-style-ชนะ-class ซ้ำในไฟล์เดิม
- **เกิดอะไร:** ตั้ง `.hero-break { margin-top }` แล้วส่งเข้า `<Text className="hero-break">` เพื่อเว้นจังหวะระหว่างกลุ่มข้อความ — วัดแล้ว gap ยัง 2px ทุกคู่ `marginTop` ยังเป็น `0px` · เป็นกับดักตัวเดียวกับที่บันทึกไว้เมื่อ 2026-08-23 (`Text` เขียน `color` inline) แค่คนละ property และอยู่ในไฟล์เดียวกันเป๊ะ
- **ทำไม:** จำบทเรียนไว้เฉพาะ "สี" ไม่ได้เก็บเป็นกฎว่า **`Text` เขียน style ทั้งชุดแบบ inline** (`margin` `font-*` `color` `text-align`) — พอเจอ property ใหม่เลยไม่นึกถึง
- **ครั้งหน้าทำยังไง:** กฎคือ **ห้ามจัด style ให้ `Text` / `Badge` / `Button` ผ่าน className เลยไม่ว่า property ไหน** — ใช้ prop `style` อย่างเดียว (Text spread `...style` ท้ายสุดจึงชนะ) · เวลาแก้ layout ให้วัด `getBoundingClientRect` ยืนยันทุกครั้ง อย่าเชื่อว่าเขียน class แล้วติด
